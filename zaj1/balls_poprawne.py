...
ELASTICITY = 0.9  # 1.0 = perfectly elastic
DRAG = 0.001  # velocity retention per second
GRAVITY = np.array([0, 200])  # pixels/second^2
...
class Ball:
    ...
    def update(self, dt, force):
        force = force - self.v * DRAG

        old_v = self.v.copy()
        self.v += force * dt
        self.x += (self.v + old_v) / 2 * dt
    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            if d < min_dist:
                ...
                a.v, b.v = a.v - ((a.v - b.v) * norm_dx).sum() * norm_dx, b.v - ((b.v - a.v) * norm_dx).sum() * norm_dx
...
def main():
    ...
    while running:
        ...
        if clicked_timer > 0:
            force += np.array([0, -4000])  # upward force when mouse clicked
            clicked_timer -= 1

        # Calculate total energy for display
        kinetic_energy = sum(0.5 * np.dot(b.v, b.v) for b in balls)
        potential_energy = sum(GRAVITY[1] * (HEIGHT - b.x[1]) for b in balls)
        ...
        for b in balls:
            b.draw(screen)
        # UI hint
        draw_text(screen, f"LBM: add force upward | A: add ball |  R: reset  |  ESC/Q: quit \n energy: {energy}", 14, (200, 200, 210), (10, 10))
